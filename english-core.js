const WORD_KEY='dudu_vocab_v2',OLD_KEY='dudu_vocab_v1',HISTORY_KEY='dudu_vocab_history_v2',DAILY_KEY='dudu_vocab_daily_v1',SETTINGS_KEY='dudu_vocab_settings_v1';
const DAY=86400000,INTERVALS=[0,1,3,7,14,30,60,120];
const LOCAL_ZH={apple:'苹果',beautiful:'美丽的',because:'因为',different:'不同的',important:'重要的',remember:'记得',question:'问题',together:'一起',family:'家庭',friend:'朋友',school:'学校',teacher:'老师',student:'学生',happy:'高兴的',sad:'难过的',water:'水',animal:'动物',morning:'早晨',afternoon:'下午',evening:'晚上',favorite:'最喜欢的',colour:'颜色',color:'颜色'};
const SPECIAL={
 beautiful:{syllables:['beau','ti','ful'],chunks:[['b',0],['eau',1],['ti',1],['ful',0]],tip:'beauty 加 -ful 时，y 变 i；eau 的读音和拼写需要整体记。'},because:{syllables:['be','cause'],chunks:[['be',0],['cause',1]],tip:'先抓住 cause（原因）；because 中间不是 cos。'},different:{syllables:['dif','fer','ent'],chunks:[['dif',0],['fer',0],['ent',0]],tip:'中间的 f 双写，结尾是 -ent。'},important:{syllables:['im','por','tant'],chunks:[['im',0],['por',0],['tant',1]],tip:'按 im-por-tant 三段读；最后是 -tant，不是 -tent。'},remember:{syllables:['re','mem','ber'],chunks:[['re',0],['mem',0],['ber',0]],tip:'重音在 mem；分三段边读边写。'},question:{syllables:['ques','tion'],chunks:[['qu',0],['es',0],['tion',0]],tip:'qu 常读 /kw/；-tion 在这里整体读 /ʃən/。'},together:{syllables:['to','geth','er'],chunks:[['to',0],['geth',1],['er',0]],tip:'注意中间是 geth，不是 gath；th 作为一个字母组合。'},apple:{syllables:['ap','ple'],chunks:[['a',0],['pp',0],['le',0]],tip:'中间 p 双写；结尾辅音 + le 常形成一个音节。'},family:{syllables:['fam','i','ly'],chunks:[['fam',0],['i',0],['ly',0]],tip:'按 fam-i-ly 三段拼，不要漏掉中间的 i。'},favorite:{syllables:['fa','vor','ite'],chunks:[['fa',0],['vor',0],['ite',1]],tip:'美式拼写 favorite；结尾 -ite 的实际发音较弱，需要整体记。'}
};
const PATTERNS=[['tion','/ʃən/'],['sion','/ʒən/ 或 /ʃən/'],['ture','/tʃə/'],['eigh','/eɪ/'],['igh','/aɪ/'],['dge','/dʒ/'],['ph','/f/'],['sh','/ʃ/'],['ch','/tʃ/'],['th','/θ/ 或 /ð/'],['wh','/w/'],['ck','/k/'],['ng','/ŋ/'],['qu','/kw/'],['ee','长音 /iː/'],['ea','常见 /iː/'],['oa','/əʊ/'],['ai','/eɪ/'],['ay','/eɪ/'],['oo','/uː/ 或 /ʊ/'],['ou','多种读音'],['ow','/aʊ/ 或 /əʊ/'],['oi','/ɔɪ/'],['oy','/ɔɪ/'],['ar','/ɑː/'],['or','/ɔː/'],['er','/ə/ 或 /ɜː/'],['ir','/ɜː/'],['ur','/ɜː/'],['aw','/ɔː/'],['au','/ɔː/'],['ew','/juː/ 或 /uː/'],['ing','/ɪŋ/'],['ed','过去式结尾'],['ly','副词结尾'],['ful','充满……的'],['less','没有……的'],['ment','名词结尾'],['ness','名词结尾']];
const IRREGULAR_COMMON=new Set(['one','two','said','says','does','done','come','some','have','give','love','move','who','whole','could','would','should','people','friend','again','busy','women','answer','write','know','island','beautiful','because','favorite']);
...
function accuracyOf(w){return w.seen?Math.round((w.correct/w.seen)*100):0}function isWeak(w){return !!w.weak||(w.seen>0&&((w.level||0)<3||accuracyOf(w)<75))}function isMastered(w){return (w.level||0)>=5&&(w.spellStreak||0)>=2&&!w.weak}function dueWords(){return words.filter(w=>w.seen>0&&(w.due||0)<=day()).sort((a,b)=>(a.due||0)-(b.due||0)||(a.level||0)-(b.level||0))}function weakWords(){return words.filter(isWeak).sort((a,b)=>accuracyOf(a)-accuracyOf(b)||(a.level||0)-(b.level||0))}
function newWordsPreview(){const limit=settings.dailyLimit||8,already=words.filter(w=>w.introducedDay===day()),slots=Math.max(0,limit-already.length),candidates=words.filter(w=>w.seen===0&&w.introducedDay==null).sort((a,b)=>a.createdAt-b.createdAt).slice(0,slots);return [...already,...candidates].slice(0,limit)}
function smartWords(){const out=[],seen=new Set();[...weakWords().slice(0,5),...dueWords().slice(0,5),...newWordsPreview().slice(0,4)].forEach(w=>{if(!seen.has(w.id)){seen.add(w.id);out.push(w)}});return out.slice(0,12)}
...
function phonicsChunks(word){const out=[];let i=0;while(i<word.length){let found=null;for(const[p]of PATTERNS){if(word.slice(i,i+p.length)===p){found=p;break}}if(found){out.push([found,['ou','ow','ea','oo'].includes(found)?1:0]);i+=found.length}else{let j=i+1;while(j<word.length&&!PATTERNS.some(([p])=>word.slice(j,j+p.length)===p))j++;const raw=word.slice(i,j);for(let k=0;k<raw.length;k++)out.push([raw[k],0]);i=j}}return out}
function withTimeout(url,ms=6500){const c=new AbortController(),t=setTimeout(()=>c.abort(),ms);return fetch(url,{signal:c.signal}).finally(()=>clearTimeout(t))}
